...
<body>
	...
	<div id="content">
		<div class="container-fluid">
			<h1 class="page-title">
				日程安排 <small>Personal information</small>
			</h1>
			<div class="row-fluid">
				<div class="span12">
					<div class="widget">
						...
						<div class="widget-content">
							<ul class="nav nav-tabs">
								<li class="active"><a href="#overview" data-toggle="tab"><i
										class="icon-desktop"></i> <span class="hidden-phone">日程安排</span></a></li>
								<li><a href="#edit-profile" data-toggle="tab"><i
										class="icon-edit"></i> <span class="hidden-phone">添加日程</span></a></li>
							</ul>
							<div class="tab-content">
								<div class="tab-pane fade active in" id="overview">
									<div class="user-profile">
										<div class="profile-header">
											<i class="icon-info-sign"></i> 日程安排
										</div>
																...
																<c:forEach items="${works}" var="work">
																	<tr>
																		...
																		<td><a
																			...
																			href="servlet/DeleteWorkServlet?id=${work.id }"
																			onclick="return confirm('你确定要删除吗？');"><span
																				class="label">删除</span></a></td>
																	</tr>
																</c:forEach>
															</c:when>
															<c:otherwise>
								...
								<div class="tab-pane fade" id="edit-profile">
									<div class="user-profile">
										<form action=servlet/AddWorkServlet method="post">
											<div class="profile-header">
												<i class="icon-cogs"></i> 添加日程
											...
											<div class="control-group">
												...
												<div class="controls">
													<jsp:include page="/pages/common/select.jsp">
														<jsp:param
															value="select e.id,e.name from t_employee w inner join t_writer we on w.id = we.writer_id inner join t_employee e on e.id = we.emp_id where w.id = ? union select e.id,e.name from t_employee e where e.id = ?"
															name="sql" />
														<jsp:param value="${current.id }" name="param" />
														<jsp:param value="${current.id }" name="param" />
														<jsp:param value="emp" name="name" />
														<jsp:param value="${current.id }" name="value" />
													</jsp:include>
												</div>
											</div>
											<div>
												<label class="control-label">开始时间</label> <input
													name="startTime" type="text" id="control_date2" size="10"
													maxlength="10" onClick="new Calendar().show(this);"
													readonly="readonly" />
											</div>
											<div>
												<label class="control-label">结束时间</label> <input
													name="endTime" type="text" id="control_date2" size="10"
													maxlength="10" onClick="new Calendar().show(this);"
													readonly="readonly" />
											</div>
											...
											<div class="control-group">
												<div class="widget">
													...
													<div class="widget-content">
														<textarea class="textarea input-block-level"
															id="wysihtml5" name="content" placeholder="输入内容 ..."
															rows="12"></textarea>
													</div>
												</div>
											</div>
